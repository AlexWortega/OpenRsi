    ...
    int L, int* isel)
{
  ...
  for(int b=0;b<4;b++){
    ...
    if(isK){
      ...
      for(int n=gt;n<CDIM;n+=gT){ Q[n]=0.f; Kk[n]=0.f; Vv[n]=0.f; Gg[n]=0.f; }
      grid.sync();
      ...
      int MQD=MH*(QN+QR); int KVAD=KVL+QR;
      for(int n=gt;n<MQD;n+=gT) MQ[n]=0.f;
      for(int n=gt;n<KVAD;n+=gT) KVA[n]=0.f;
      grid.sync();
      ...
      grid.sync();
      // ---- qc[h,i] = sum_d Wk[i,h*256+d] * q_nope[h,d] ----
      pu8 bwq=(pu8)qw[(qi+2)*3+0]; pbf bsc=(pbf)qw[(qi+2)*3+1]; pbf bzr=(pbf)qw[(qi+2)*3+2];
      int KVBN=MH*(QN+VH);   // 8192
      for(int idx=gt; idx<MH*KVL; idx+=gT){
        int h=idx/KVL, i=idx%KVL;
        float acc=0.f; int colb=h*(QN+VH);
        for(int d=0; d<QN; d++) acc += dqe(bwq,bsc,bzr,i,colb+d,KVBN)*MQ[h*(QN+QR)+d];
        QC[h*KVL+i]=acc;
      }
      grid.sync();
      ...
      __syncthreads();
      // One token per block-iteration: load ckv[l]+krope[l] to shared (1 barrier),
      // then 16 threads/head split the 512+64 dot and reduce via WARP SHUFFLE
      // (no second barrier -- the old per-token hot spot). 32 heads x 16 = 512 thr.
      float* ckvrow=smem+MH*KVL;       // KVL floats
      float* krow=ckvrow+KVL;          // QR floats
      const int TPH=16; int myh=tid/TPH, sub=tid%TPH;
      for(int l=bid; l<Lp; l+=nb){
        for(int i=tid;i<KVL;i+=bs) ckvrow[i]=(l<L)?__bfloat162float(ckv[l*KVL+i]):KVA[i];
        for(int i=tid;i<QR;i+=bs) krow[i]=(l<L)?__bfloat162float(krope[l*QR+i]):KRN[i];
        __syncthreads();
        float acc=0.f; const float* qch=qccache+myh*KVL;
        for(int i=sub;i<KVL;i+=TPH) acc += qch[i]*ckvrow[i];
        if(sub<QR){ const float* qrh=MQ+myh*(QN+QR)+QN; acc += qrh[sub]*krow[sub]; }
        #pragma unroll
        for(int o=8;o>0;o>>=1) acc+=__shfl_down_sync(0xffffffff,acc,o);
        if(sub==0) SCO[l*MH+myh]=acc*MLA_SCALE;
      }
      grid.sync();
      // ---- softmax over l per head (one head per block) ----
      for(int h=bid; h<MH; h+=nb){
        float loc=-1e30f;
        for(int l=tid;l<Lp;l+=bs) loc=fmaxf(loc,SCO[l*MH+h]);
        float mx=blockMax(loc,rsm,tid,bs);
        float ls=0.f;
        for(int l=tid;l<Lp;l+=bs){ float e=expf(SCO[l*MH+h]-mx); SCO[l*MH+h]=e; ls+=e; }
        float sm=blockSum(ls,rsm,tid,bs);
        float inv=1.f/sm;
        for(int l=tid;l<Lp;l+=bs) SCO[l*MH+h]*=inv;
      }
      grid.sync();
      ...
      { const int TB=8;               // tokens per barrier pair (amortizes syncs)
        ...
        for(int lb=bid*TB; lb<Lp; lb+=nb*TB){
          ...
          for(int t2=0;t2<nt;t2++){ int l=lb+t2;
            ...
            for(int h=tid;h<MH;h+=bs) prow[t2*MH+h]=SCO[l*MH+h];
          }
          __syncthreads();
      ...
      grid.sync();
      // ---- o[h,d]=sum_i pc[h,i]*Wv[i,h*256+128+d] ----
      float* Ob=work+O_O;
      for(int idx=gt; idx<MH*VH; idx+=gT){
        int h=idx/VH, d=idx%VH;
        float acc=0.f; int colb=h*(QN+VH)+QN;
        const float* pch=PC+h*KVL;
        for(int i=0;i<KVL;i++) acc += pch[i]*dqe(bwq,bsc,bzr,i,colb+d,KVBN);
        Ob[h*VH+d]=acc;
      }
      grid.sync();
    ...
    // ===================== MoE =====================
    {
      ...
      grid.sync();
      // softmax + topk8 on block 0 thread 0
      if(bid==0 && tid==0){
        float mx=-1e30f; for(int e=0;e<NE;e++) mx=fmaxf(mx,RL[e]);
        float s=0.f; for(int e=0;e<NE;e++){ float v=expf(RL[e]-mx); PR[e]=v; s+=v; }
        for(int e=0;e<NE;e++) PR[e]/=s;
        // topk8
        float used[NE]; for(int e=0;e<NE;e++) used[e]=PR[e];
        float wsum=0.f;
        for(int j=0;j<NA;j++){
          int be=0; float bv=-1.f;
          for(int e=0;e<NE;e++) if(used[e]>bv){bv=used[e];be=e;}
          isel[j]=be; WSEL[j]=bv; used[be]=-1.f; wsum+=bv;
        }
        float nrm=ROUTED_SCALING/(wsum+1e-9f);
        for(int j=0;j<NA;j++) WSEL[j]*=nrm;
      }
      grid.sync();
      ...
      { int N4=MINT>>2; int Nw=MINT>>2; long per=(long)N4*NGH; long unit=(NA+1)*per; long tot=2*unit;
        for(long id=gt; id<tot; id+=gT){
          ...
          pu8 wq; pbf sc,zr; float* out;
          if(!isup){ out=HEXP; if(slot<NA){int e=isel[slot];wq=gwq+e*gstr;sc=gsc+e*gsstr;zr=gzr+e*gsstr;}else{wq=sgwq;sc=sgsc;zr=sgzr;} }
          else { out=OEXP; if(slot<NA){int e=isel[slot];wq=uwq+e*gstr;sc=usc+e*gsstr;zr=uzr+e*gsstr;}else{wq=suwq;sc=susc;zr=suzr;} }
      ...
      grid.sync();
      // combine silu(gate)*up into HEXP
      for(int i=gt;i<(NA+1)*MINT;i+=gT){ float hg=HEXP[i]; HEXP[i]=(hg/(1.f+expf(-hg)))*OEXP[i]; }
      grid.sync();
      ...
      for(int i=gt;i<(NA+1)*HID;i+=gT) OEXP[i]=0.f;
      grid.sync();
      { int NGM=MINT>>7; int N4=HID>>2; int Nw=HID>>2; long per=(long)N4*NGM; long tot=(long)(NA+1)*per;
        for(long id=gt; id<tot; id+=gT){
          int slot=id/per; long r=id%per; int c4=r%N4; int g=r/N4; int d=c4<<2;
          pu8 wq; pbf sc,zr; const float* hin2=HEXP+slot*MINT;
          if(slot<NA){int e=isel[slot];wq=dwq+e*dstr;sc=dsc+e*dsstr;zr=dzr+e*dsstr;}else{wq=sdwq;sc=sdsc;zr=sdzr;}
          const unsigned int* w32=(const unsigned int*)(wq+(g*64)*HID+d);
          float d0=0,d1=0,d2=0,d3=0,xsum=0.f;
          #pragma unroll 8
          for(int t=0;t<64;t++){ unsigned int bb=w32[t*Nw]; int k=g*128+2*t; float x0=hin2[k],x1=hin2[k+1]; xsum+=x0+x1;
            d0+=x0*(float)(bb&0xF)+x1*(float)((bb>>4)&0xF); d1+=x0*(float)((bb>>8)&0xF)+x1*(float)((bb>>12)&0xF);
            d2+=x0*(float)((bb>>16)&0xF)+x1*(float)((bb>>20)&0xF); d3+=x0*(float)((bb>>24)&0xF)+x1*(float)((bb>>28)&0xF); }
      ...
      for(int d=gt;d<HID;d+=gT){
        float acc=0.f;
        for(int slot=0;slot<NA;slot++) acc+=WSEL[slot]*OEXP[slot*HID+d];
