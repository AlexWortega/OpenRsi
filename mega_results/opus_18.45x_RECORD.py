...
single-wave, latency-bound weight GEMVs. So the hot path was tuned by
  ...
  - streaming the int4 weights with __ldcs (evict-first / streaming cache hint)
    on the three big strided uint32 GEMVs (KDA q/k/v/g, MoE gate/up, MoE down):
    the packed weights are single-use per token, so evict-first stops them
    from polluting L2 and keeps the REUSED activations (XN) and the MLA kv_b /
    ckv rows resident -- the single biggest win (~15.5x -> ~18.4x geomean).
    Applying __ldcs to the narrow byte-per-thread o_proj GEMV or the reused
    dqe/ckv reads instead HURTS (they want normal caching), so the hint is
    surgically limited to the three streaming GEMVs;
  - reading the MLA ckv cache as bf16 PAIRS (__nv_bfloat162 / float2) in the
    scores and pc phases: wider 64-bit transactions on the growing latent cache
    cut the long-context cost (16k ctx ~15.7x -> ~16.2x);
    ...
    if(isK){
      ...
      { int NG=HID>>7; int N4=CDIM>>2; long tot=(long)N4*NG; int Nw=CDIM>>2;
        for(long id=gt; id<4*tot; id+=gT){
          ...
          float d0=0,d1=0,d2=0,d3=0,xsum=0.f;
          #pragma unroll 8
          for(int t=0;t<64;t++){ unsigned int b=__ldcs(&w32[t*Nw]); int k=g*128+2*t; float x0=XN[k],x1=XN[k+1]; xsum+=x0+x1;
      ...
      for(int l=bid; l<Lp; l+=nb){
        float acc=0.f;
        if(l<L){ const __nv_bfloat162* cr2=(const __nv_bfloat162*)(ckv+(long)l*KVL);
                 // each thread reads bf16 PAIRS; 16 threads*2 = 32 consecutive bf16 = 64B coalesced
                 for(int i=sub;i<KVL/2;i+=TPH){ float2 wf=__bfloat1622float2(cr2[i]);
                   acc += qch[2*i]*wf.x + qch[2*i+1]*wf.y; } }
      ...
      { const int TB=8;               // tokens per barrier pair (amortizes syncs)
        ...
        for(int lb=bid*TB; lb<Lp; lb+=nb*TB){
          ...
          for(int t2=0;t2<nt;t2++){ int l=lb+t2;
            if(l<L){ const __nv_bfloat162* c2=(const __nv_bfloat162*)(ckv+(long)l*KVL);
                     float2* cr2=(float2*)(crow+t2*KVL);
                     for(int i=tid;i<KVL/2;i+=bs) cr2[i]=__bfloat1622float2(c2[i]); }
            else   { for(int i=tid;i<KVL;i+=bs) crow[t2*KVL+i]=KVA[i]; }
    ...
    // ===================== MoE =====================
    {
      ...
      { int N4=MINT>>2; int Nw=MINT>>2; long per=(long)N4*NGH; long unit=(NA+1)*per; long tot=2*unit;
        for(long id=gt; id<tot; id+=gT){
          ...
          float d0=0,d1=0,d2=0,d3=0,xsum=0.f;
          #pragma unroll 8
          for(int t=0;t<64;t++){ unsigned int bb=__ldcs(&w32[t*Nw]); int k=g*128+2*t; float x0=XN[k],x1=XN[k+1]; xsum+=x0+x1;
      ...
      { int NGM=MINT>>7; int N4=HID>>2; int Nw=HID>>2; long per=(long)2*N4*NGM; long tot=(long)(NA+1)*per;
        for(long id=gt; id<tot; id+=gT){
          ...
          #pragma unroll 8
          for(int t=t0h;t<t1h;t++){ unsigned int bb=__ldcs(&w32[t*Nw]); int k=g*128+2*t; float x0=hin2[k],x1=hin2[k+1]; xsum+=x0+x1;
            d0+=x0*(float)(bb&0xF)+x1*(float)((bb>>4)&0xF); d1+=x0*(float)((bb>>8)&0xF)+x1*(float)((bb>>12)&0xF);
            d2+=x0*(float)((bb>>16)&0xF)+x1*(float)((bb>>20)&0xF); d3+=x0*(float)((bb>>24)&0xF)+x1*(float)((bb>>28)&0xF); }
...
class Model(nn.Module):
    ...
    # workspace offsets for the appended MLA latent row (match the C offsets)
    _O_KVA = 2304 + 2304 + 4096 * 5 + 2304 + 32 * 192
    _O_KRNEW = _O_KVA + (512 + 64) + 32 * 512 + 32 * 512

    def _setup_state(self, state, dev, L):
        """One-time per state-instance setup: capacity-backed MLA cache (no
        per-step O(context) torch.cat realloc), a cached state-pointer table
        (all state pointers are now stable across steps), and a preallocated
        workspace. Extra keys are stashed inside the MLA state dict so identity
        travels with the state object the harness threads back into step()."""
        cfg = self.cfg
        st3 = state[3]
        cap = L + 1024  # room for far more decode steps than any sweep uses
        cap_ckv = torch.empty(cap, cfg.kv_lora, device=dev, dtype=torch.bfloat16)
        cap_kr = torch.empty(cap, cfg.qk_rope, device=dev, dtype=torch.bfloat16)
        cap_ckv[:L].copy_(st3["c_kv"])
        cap_kr[:L].copy_(st3["k_rope"])
        st3["_cap_ckv"] = cap_ckv
        st3["_cap_kr"] = cap_kr
        st3["_cap"] = cap
        st3["c_kv"] = cap_ckv[:L]
        st3["k_rope"] = cap_kr[:L]
        ...
        for b in range(4):
            ...
            if self.blocks[b].kind == "K":
                stp += [s["S"].data_ptr(), s["cq"].data_ptr(), s["ck"].data_ptr(), s["cv"].data_ptr()]
            else:
                stp += [s["_cap_ckv"].data_ptr(), s["_cap_kr"].data_ptr(), 0, 0]
        st3["_stp"] = torch.tensor(stp, dtype=torch.int64, device=dev)
        wsize = (12 * 1024 + 200000) + (cap + 1) * 32 + 4096
        st3["_work"] = torch.empty(int(wsize), dtype=torch.float32, device=dev)
        # double-buffered output so hin (== previous hout) never aliases the
        # current hout target.
        st3["_hbuf"] = [torch.empty(cfg.hidden, dtype=torch.bfloat16, device=dev),
    ...
    def step(self, hidden, state):
        ...
        st3 = state[3]
        L = st3["c_kv"].shape[0]
        # (Re)build the capacity cache on a fresh state instance, or if the
        # decode overran the reserved capacity.
        if "_cap" not in st3 or L >= st3["_cap"]:
            self._setup_state(state, dev, L)

        stp = st3["_stp"]
        work = st3["_work"]
        st3["_hidx"] ^= 1
        hout = st3["_hbuf"][st3["_hidx"]]
        # hidden is already the previous step's bf16/contiguous hout (or the
        # bf16 init token); only convert on the rare off-format input.
        if hidden.dtype == torch.bfloat16 and hidden.is_contiguous():
            hin = hidden
        else:
        ...
        # append the new MLA latent row into the preallocated capacity slot
        cap_ckv = st3["_cap_ckv"]
        cap_kr = st3["_cap_kr"]
        cap_ckv[L].copy_(work[self._O_KVA:self._O_KVA + cfg.kv_lora])
        cap_kr[L].copy_(work[self._O_KRNEW:self._O_KRNEW + cfg.qk_rope])
        st3["c_kv"] = cap_ckv[:L + 1]
        st3["k_rope"] = cap_kr[:L + 1]
        return hout, state
