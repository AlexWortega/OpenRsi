...
After a transient orchestration/API failure, resume the same directory without repaying completed stages:

```bash
OPENRSI_PROOFS_DIR=/absolute/path/to/run \
OPENRSI_PROOFS_BUDGET_USD=30 \
node --env-file=.env dist/proofs/run7.js --resume
```
