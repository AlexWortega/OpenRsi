/**
 * run10: run9 (goal-directed roadmap + Lean channel + scout + subagents) plus three
 * mechanisms ported from prime-agent:
 *  - a prime-agent Sol engine (OPENRSI_SOL_ENGINE=prime) whose worker gets a persistent
 *    IPython kernel and recursive rlm() subagents, driven by an autonomous loop whose
 *    completion gate is scripts/gate_verify.py — the worker cannot declare victory until
 *    the same checks the harness re-runs independently actually pass;
 *  - a workspace fingerprint, so a generation that changed nothing is killed immediately
 *    instead of paying two reviewers to discuss an empty packet;
 *  - a wind-down phase: when the remaining budget can no longer fund a generation, the
 *    campaign spends its reserve writing HANDOFF.md instead of dying mid-ideation.
 */
...
const SOL_ENGINE = process.env.OPENRSI_SOL_ENGINE || "pi";
const PRIME_BIN = process.env.OPENRSI_PRIME_BIN || join(process.env.HOME || "/root", ".local-npm/bin/prime-agent");
const PRIME_PROVIDER = process.env.OPENRSI_PRIME_PROVIDER || "azure-openai-responses";
const PRIME_MODEL = process.env.OPENRSI_PRIME_MODEL || "gpt-5.6-sol";
const PRIME_TIMEOUT_MS = positiveIntegerEnv("OPENRSI_PRIME_TIMEOUT_MS", 90 * 60_000);
...
/**
 * Content fingerprint of everything a generation could plausibly produce. Ported from
 * prime-agent's git-worktree gate snapshot, but hashing tracked files directly since a
 * run directory is not always a git repo.
 */
function workspaceFingerprint(runDir: string): string {
  ...
  const walk = (dir: string, depth: number): void => {
    ...
    for (const name of entries) {
      if (name === "prior" || name === ".session" || name.startsWith(".") || name === "__pycache__") continue;
...
/** Bill a prime-agent JSON event stream at the worker model's token rates. */
...
/**
 * Run one generation's implementation turn with prime-agent: persistent IPython kernel,
 * rlm() subagents, and an autonomous loop gated on scripts/gate_verify.py, so the worker
 * keeps going until the packet it wrote actually verifies. Returns its USD cost.
 */
function runPrimeWorker(
  repoRoot: string,
  runDir: string,
  genDir: string,
  packetPath: string,
  ...
  const jsonl = join(genDir, "prime_events.jsonl");
  const gate = `${process.env.OPENRSI_PYTHON || "python3"} ${join(repoRoot, "scripts", "gate_verify.py")} ${runDir} ${packetPath}`;
  const maxTokens = Math.max(50_000, Math.min(PRIME_MAX_TOKENS, Math.floor((budgetUsd * 1e6) / PRIME_RATE_OUT)));
  const args = [
    "--print", "--mode", "json",
    "--cwd", runDir,
    "--provider", PRIME_PROVIDER,
    "--model", PRIME_MODEL,
    "--thinking", process.env.OPENRSI_MLXFAST_THINKING || "high",
    "--session-dir", join(genDir, "prime_session"),
    "--autonomous",
    "--autonomous-gate", gate,
    "--autonomous-gate-retries", "6",
    "--autonomous-gate-timeout-ms", String(VERIFIER_TIMEOUT_MS + LEAN_TIMEOUT_MS),
    "--autonomous-max-turns", String(PRIME_MAX_TURNS),
    "--autonomous-max-tokens", String(maxTokens),
    "--autonomous-max-continuations", "40",
    "--autonomous-timeout-ms", String(PRIME_TIMEOUT_MS),
    prompt,
  ];
  return new Promise((resolvePromise) => {
    const out = openSync(jsonl, "a");
    const err = openSync(join(genDir, "prime_worker.err"), "a");
    const child = spawn(PRIME_BIN, args, {
      cwd: runDir,
      env: { ...process.env, OPENRSI_LEAN_PROJECT: LEAN_PROJECT, OPENRSI_LAKE: LAKE },
      stdio: ["ignore", out, err],
    });
    const timer = setTimeout(() => {
      child.kill("SIGTERM");
      setTimeout(() => { if (child.exitCode === null) child.kill("SIGKILL"); }, 30_000).unref();
    }, PRIME_TIMEOUT_MS + 5 * 60_000);
    const finish = () => {
      clearTimeout(timer);
      closeSync(out);
      closeSync(err);
      resolvePromise(primeStreamCost(jsonl));
    };
...
async function main(): Promise<void> {
  ...
  // Every stage also leaves the wind-down reserve untouched, so the campaign can always
  // afford to write its own handoff.
  const canReserve = (usd: number) => budget - spent() >= usd + WINDDOWN_USD;
  checkpoint();
  ...
  researchLoop: for (let generation = firstGeneration; generation <= MAX_GENERATIONS; generation++) {
    ...
    const fingerprintBefore = workspaceFingerprint(runDir);
    if (!existsSync(resultPacket) && SOL_ENGINE === "prime") {
      await runPrimeWorker(repoRoot, runDir, genDir, resultPacket,
        `${SOL_SYSTEM}\n\nGeneration ${generation}. ${sharedBrief} Implement and adversarially verify the single best surviving bounded experiment against the roadmap frontier. You have a persistent IPython kernel — keep exact search state in it across turns instead of recomputing — and rlm() subagents; a builder/prover/breaker split is productive. Write the result packet to ${resultPacket}; an autonomous gate re-runs every verifier you name, so the run does not finish until they actually pass.`,
        Math.max(1, budget - spent()),
      ).then((cost) => { subagentCost += cost; });
      checkpoint();
    } else if (!existsSync(resultPacket) && SOL_SUBAGENTS) {
    ...
    // A generation that changed nothing has nothing to review; killing it here saves the
    // two reviewer calls that would otherwise discuss an empty packet.
    if (workspaceFingerprint(runDir) === fingerprintBefore) {
      atomicJson(join(genDir, "GATE.json"), {
        decision: "KILL_IDEA", stage: "no_workspace_change",
        reason: "the implementation turn left the workspace byte-identical; no experiment was run",
      });
      state = { ...state, phase: "gate", decision: "KILL_IDEA", reason: "implementation turn produced no workspace change" };
      state.kills_since_progress = (state.kills_since_progress ?? 0) + 1;
      checkpoint();
      event({ phase: state.phase, generation, status: "completed", decision: "KILL_IDEA", reason: "no_workspace_change", spent_usd: spent() });
      continue researchLoop;
    }
  ...
  // Wind-down: prior campaigns died mid-ideation with a live candidate and nobody to
  // write it down. Spend the reserve on a handoff instead of on a generation that cannot
  // finish anyway.
  const handoffPath = join(runDir, "HANDOFF.md");
  if (!existsSync(handoffPath) && spent() < budget) {
    state = { ...state, phase: "winddown" };
    checkpoint();
    event({ phase: "winddown", status: "started", spent_usd: spent() });
    try {
      const preamble = solPrimed ? "" : `${SOL_SYSTEM}\n\n`;
      await session.prompt(`${preamble}The campaign is out of budget and this is its final turn. Do not start new substantive work and do not claim a result the verifiers do not already certify. Write HANDOFF.md covering, in this order: the target theorem and how far it actually got; every result that is machine-checked, each with the verifier or Lean file that certifies it; every candidate still alive when funding ran out, with exactly what the next experiment on it should be; the obstructions that killed the rest, so a later campaign does not re-enter them; and the current frontier lemma with an honest assessment of why it resisted. Then make sure STATUS.md and ROADMAP.md agree with it.`);
      await session.waitForIdle();
      event({ phase: "winddown", status: "completed", spent_usd: spent() });
    } catch (error) {
      event({ phase: "winddown", status: "failed", reason: error instanceof Error ? error.message : String(error) });
    }
  }
