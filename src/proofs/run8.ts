/**
 * run8: goal-directed proof search. run7's model-separated generation loop, but the
 * campaign first synthesizes a ROADMAP.md — an end-to-end chain of intermediate lemmas
 * ending at the target theorem — and every generation attacks the roadmap's FRONTIER
 * lemma instead of ideating unconstrained. Consecutive frontier kills trigger a
 * roadmap revision that reroutes around the dead lemma.
 */
    ...
    child.once("close", (code, signal) => {
      ...
      if (stdout.trim().length < 200) {
        ...
        if (args.includes("--ideate") || args.includes("--roadmap")) {
          ...
          writeFileSync(`${outputPath}.stderr`, `${stderr}\n[proofs8] proposer response remained truncated after retry; peer review will reject it.\n`);
          resolvePromise();
          return;
        }
...
async function main(): Promise<void> {
  ...
  if (!existsSync(roadmapPath)) {
    ...
    const seedContext = ["ORACLE_BRIEF.md", "IDEAS.md", "STATUS.md", "LITERATURE.md", "NOTES.md"];
    ...
    if ((state.kills_since_progress ?? 0) >= REPLAN_KILLS) {
      ...
      if (!existsSync(revision)) {
        await runOracle(python, runDir, revision, [askPro, "--roadmap", "--model", PRO,
          `The current roadmap's frontier lemma has now been killed ${state.kills_since_progress} consecutive generations. Rewrite the roadmap: keep the target theorem fixed, absorb every obstruction recorded since the last revision, and either reroute around the dead frontier or replace the strategy entirely. Output the complete replacement ROADMAP.md.`,
          "ROADMAP.md", "STATUS.md", "IDEAS.md", "NOTES.md"]);
      }
...
main().catch((error) => {
  process.stderr.write(`[proofs8] FATAL ${error instanceof Error ? error.stack : String(error)}\n`);
  process.exitCode = 1;
});
