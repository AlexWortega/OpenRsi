      ...
      if(col4<ncol4){
        ...
        for(int su=su0; su<su1; ++su){
            ...
            for(int r=r0; r<r0+chunk/2; ++r){
                uint32_t w=__ldg(&wq32[r*Nw+col4]); float xa=g_xsh[2*r-kbeg], xb=g_xsh[2*r+1-kbeg];
                xs+=xa+xb;
...
__device__ void kda_attn(const int64_t* W, const int64_t* St, float* hid, float* scr, cg::grid_group& grid){
    ...
    __syncthreads();
    // NO-REDUCE bgemv: conv folds the ksplit reduction inline -> saves a grid.sync.
    int qkvg_ks=bgemv_i4_nr(sxs,swq,ssc,szr,q,D,C,4,grid); PROFT(1);
    ...
    // State update: one block per head, but 2 threads per column (i-halves) so
    // all 256 threads (8 warps) work -> better latency hiding on the dependent
    // i-reduction than the 128-thread version. gk[i]=gexp*k precomputed.
    for(int h=blockIdx.x; h<HK; h+=gridDim.x){
        float* Sh=Sst+h*DK*DK;
        __shared__ float qh[DK],kh[DK],gh[DK],gk[DK],vh[DK],pred[DK]; __shared__ float betas;
        __shared__ float ppart[DK][2];
        #define opart ppart
        for(int i=tid;i<DK;i+=NT){ float ge=expf(g[h*DK+i]); qh[i]=q[h*DK+i]*scale; kh[i]=k[h*DK+i]; gh[i]=ge; gk[i]=ge*k[h*DK+i]; vh[i]=v[h*DK+i]; }
        if(tid==0) betas=beta[h];
        __syncthreads();
        int j=tid&127, half=tid>>7; int i0=half*64, i1=i0+64;
        bool act2 = (half<2);   // only first 256 threads drive the 2-half scheme
        // pass 1: partial pred over i-half
        if(act2){
            float acc=0;
            #pragma unroll 4
            for(int i=i0;i<i1;i++) acc+=Sh[i*DK+j]*gk[i];
            ppart[j][half]=acc;
        }
        __syncthreads();
        if(half==0) pred[j]=ppart[j][0]+ppart[j][1];
        __syncthreads();
        // pass 2: update S[i,j] over i-half, partial o
        if(act2){
            float diff=vh[j]-pred[j]; float acc=0;
            #pragma unroll 4
            for(int i=i0;i<i1;i++){ float sij=Sh[i*DK+j]*gh[i]+betas*kh[i]*diff; Sh[i*DK+j]=sij; acc+=sij*qh[i]; }
            opart[j][half]=acc;
        }
        __syncthreads();
        if(half==0) oreg[h*DK+j]=opart[j][0]+opart[j][1];
        __syncthreads();
        #undef opart
    }
    grid.sync(); PROFT(2);
    gemv_i4(oreg,(const uint8_t*)W[14],(const bf16*)W[15],(const bf16*)W[16],aout,C,D,grid); PROFT(3);
...
__device__ void mla_attn(const int64_t* W, const int64_t* St, int L, float* hid, float* scr, cg::grid_group& grid){
    ...
    // per-warp shared row; qaT read from L2 (32KB, resident). qrope in shared.
    {
        ...
        bf16* myckv=ckv_sh+warp*L0; bf16* mykr=kr_sh+warp*QR;
        for(int l=blockIdx.x*NWARP+warp; l<Lt; l+=gridDim.x*NWARP){
            const bf16* ck=nckv+(size_t)l*L0; const bf16* kr=nkrope+(size_t)l*QR;
            for(int i=lane;i<L0;i+=32) myckv[i]=ck[i];
            for(int i=lane;i<QR;i+=32) mykr[i]=kr[i];
            __syncwarp();
            float a=0;
            #pragma unroll 16
            for(int kk=0;kk<L0;kk++) a+=b2f(__ldg(&qaT_g[(size_t)kk*H+h]))*b2f(myckv[kk]);
            #pragma unroll
            for(int dd=0;dd<QR;dd++) a+=b2f(qr_s[h*QR+dd])*b2f(mykr[dd]);
    ...
    grid.sync(); PROFT(16);
    PROFT(17);   // exp fused into cvec staging below (saved a grid.sync)
    ...
    int bph = SEG;
    {
        ...
        if(lseg < bph){
            ...
            // stage this segment's scores p[l0..l1, 0:H] in shared (fp32, <=16KB)
            float* ps=g_xsh;                    // seglen*H floats (<=129*32=4128)
            for(int idx=tid; idx<seglen*H; idx+=NT){ int h=idx%H; ps[idx]=expf(scores[(size_t)(l0+idx/H)*H + h]-hmax[h]); }
            __syncthreads();
            ...
            const int HH=H/2;
            for(int t=tid; t<L0*2; t+=NT){
                int kk=t>>1, hg=t&1; int h0=hg*HH;
                float acc[HH];
                #pragma unroll
    ...
    grid.sync(); PROFT(20);
    gemv_i4(ohead,(const uint8_t*)W[11],(const bf16*)W[12],(const bf16*)W[13],aout,H*VH,D,grid); PROFT(21);
    for(int i=gid;i<D;i+=GRID) hid[i]=b2f(__float2bfloat16(hid[i]+b2f(__float2bfloat16(aout[i]))));
    grid.sync();
}
...
__device__ void moe(const int64_t* W, float* hid, float* scr, cg::grid_group& grid){
    ...
    // warp-shuffle reduce -> no block __syncthreads tree, no idle-block waits.
    {
        ...
        for(int e=warp; e<E; e+=(GRID>>5)){
            ...
            for(int i=lane;i<D;i+=32) acc+=b2f(__float2bfloat16(xn[i]))*b2f(__ldg(&rwe[i]));
            #pragma unroll
            for(int o=16;o>0;o>>=1) acc+=__shfl_xor_sync(0xffffffff,acc,o);
    ...
    if(blockIdx.x==0 && tid<32){
        ...
        rout[tid]=p0/sm; rout[tid+32]=p1/sm;
        __syncwarp();
        if(tid==0){
            float sumw=0;
    ...
    __syncthreads();
    bgemv_i4(xs,wq,sc,zr,gbuf,D,MI,2*NEXP,grid);   // gbuf[0..NEXP*MI)=gate, [NEXP*MI..2*NEXP*MI)=up
    for(int i=gid;i<NEXP*MI;i+=GRID){ float gg=gbuf[i]; gbuf[i]=(gg/(1.f+expf(-gg)))*gbuf[NEXP*MI+i]; }
    grid.sync(); PROFT(24);
...
__global__ void __launch_bounds__(NT,2) mega_kernel(const int64_t* W, const int* Woff, const int64_t* St,
