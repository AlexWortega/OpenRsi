...
__device__ void mla_attn(const int64_t* W, const int64_t* St, int L, float* hid, float* scr, cg::grid_group& grid){
    ...
    // per-warp shared row; qaT read from L2 (32KB, resident). qrope in shared.
    {
        ...
        bf16* myckv=ckv_sh+warp*L0;
        bf16* mykr=kr_sh+warp*QR;
        for(int l=blockIdx.x*NWARP+warp; l<Lt; l+=gridDim.x*NWARP){
            ...
            for(int kk=0;kk<L0;kk+=2){ int k2=kk>>1;
                ...
                float2 c=__bfloat1622float2(ckv2[k2]); a+=q0*c.x+q1*c.y; }
            #pragma unroll
            for(int dd=0;dd<QR;dd++){ float qd=b2f(qr_s[h*QR+dd]); a+=qd*b2f(mykr[dd]); }
...
void mega_launch(int64_t Wtab, int64_t Woff, int64_t Stab,
    ...
    { size_t sc_sh=(size_t)(H*QR + (NT/32)*L0 + (NT/32)*QR)*sizeof(bf16); if(sc_sh>shmem) shmem=sc_sh; }
