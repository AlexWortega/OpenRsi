    ...
    if _MOD is None:
        ...
        _MOD = load_inline(
            ...
            extra_cuda_cflags=["-O3", "-arch=sm_120", "--use_fast_math",
                               "-DLB=" + os.environ.get("KIMI_LB", "6"),
                               "-maxrregcount=" + os.environ.get("KIMI_RREG", "40")],
            verbose=False,
        )
    return _MOD
...
class Model(nn.Module):
    ...
    def step(self, hidden, state):
        ...
        hid = self._hid
        # bf16->f32 input convert is FUSED into the megakernel (block-0 first op);
        # ensure the incoming token is contiguous bf16 for the fused read.
        hidden_bf = hidden if hidden.dtype == torch.bfloat16 else hidden.to(torch.bfloat16)
        if not hidden_bf.is_contiguous():
            hidden_bf = hidden_bf.contiguous()
        ...
        self._mod.mega_launch(
            ...
            int(hidden_bf.data_ptr()), int(self._out.data_ptr()),
        )
        # publish grown cache as views [0:L+1]
        self._cap_len = L + 1
        state[mla_i]["c_kv"] = self._cap_ckv[:L + 1]
        state[mla_i]["k_rope"] = self._cap_krope[:L + 1]
        # output bf16 convert is FUSED into the megakernel -> single launch.
