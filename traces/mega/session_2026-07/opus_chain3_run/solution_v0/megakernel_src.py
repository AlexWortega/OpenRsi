...
void mega_launch(int64_t Wtab, int64_t Woff, int64_t Stab,
                 ...
                 int64_t hidin_ptr, int64_t hidout_ptr);
...
__device__ void gemv_i4(const float* __restrict__ x, const uint8_t* __restrict__ wq,
    ...
    if(ksplit>1){
        for(int n=blockIdx.x*NT+threadIdx.x; n<N; n+=GRID){
            float acc=0;
            #pragma unroll 1
            for(int t=0;t<ksplit;t++) acc+=g_part[(size_t)t*N+n]; y[n]=acc;
        }
        grid.sync();
    }
}
...
__device__ __forceinline__ float gemv_nr_reduce(int n,int N,int ksplit){
    float acc=0;
    #pragma unroll 1
    for(int t=0;t<ksplit;t++) acc+=g_part[(size_t)t*N+n]; return acc;
}
...
// ---- batched GEMV with INLINE SiLU staging: x[j][k] = silu(gate[j*MI+k])*up[UPOFF+j*MI+k].
// Folds the SiLU(gate)*up elementwise op into the down-GEMV's shared staging so
// the separate silu pass + its grid.sync are removed. gsrc points at gbuf; each
// staged element is recomputed (cheap: 1 exp) redundantly across the blocks
// sharing the same j -- a good trade vs a full MI*NEXP r/w pass + a barrier.
__device__ void bgemv_i4_silu(const float* __restrict__ gsrc, int UPOFF,
    const uint8_t* const* wqs, const bf16* const* scs, const bf16* const* zrs,
    float* y, int K, int N, int nj, cg::grid_group& grid){
    int ncol4=N/4, ng=K/128, Nw=N/4;
    int col_blocks=(ncol4+NT-1)/NT;
    int total_cb=col_blocks*nj;
    int avail=gridDim.x/total_cb; if(avail<1) avail=1;
    ...
    int within=0,kt=0,j=0,cb=0,su0=0,su1=0,kbeg=0;
    if(active){ within=bid%total_cb; kt=bid/total_cb; j=within/col_blocks; cb=within%col_blocks;
        su0=kt*subper; su1=min(total_sub,su0+subper); kbeg=su0*chunk; int klen=(su1-su0)*chunk;
        const float* gp=gsrc+(size_t)j*K+kbeg; const float* up=gsrc+UPOFF+(size_t)j*K+kbeg;
        for(int i=threadIdx.x; i<klen; i+=NT){ float gv=gp[i]; g_xsh[i]=(gv/(1.f+__expf(-gv)))*up[i]; } }
    __syncthreads();
    if(active){
      int col4=cb*NT+threadIdx.x;
      if(j<nj && col4<ncol4){
...
__device__ __forceinline__ float nr_reduce(int j,int n,int N,int nj,int ksplit){
    ...
    float acc=0;
    #pragma unroll 1
    for(int t=0;t<ksplit;t++) acc+=g_part[((size_t)t*nj+j)*N+n]; return acc;
}
...
__device__ void kda_attn(const int64_t* W, const int64_t* St, float* hid, float* scr, cg::grid_group& grid){
    ...
    rmsnorm_b0(hid,(const bf16*)W[0],xn);
    if(blockIdx.x==0){ for(int hh=tid;hh<HK;hh+=NT) beta[hh]=0.f; }
    grid.sync();
    ...
    // shuffle reduce -> no block __syncthreads tree, no idle-block stalls.
    {
        ...
        const int NCHB=4; const int CHB=(D+NCHB-1)/NCHB;
        int warp=(blockIdx.x*NT+tid)>>5, lane=tid&31; int nw=GRID>>5;
        for(int job=warp; job<HK*NCHB; job+=nw){
            int h=job%HK, ch=job/HK; int i0=ch*CHB, i1=min(D,i0+CHB);
            const bf16* bwh=bw+h*D; float acc=0.f;
            #pragma unroll 1
            for(int i=i0+lane;i<i1;i+=32) acc+=xn[i]*b2f(__ldg(&bwh[i]));
            #pragma unroll
            for(int o=16;o>0;o>>=1) acc+=__shfl_xor_sync(0xffffffff,acc,o);
            if(lane==0) atomicAdd(&beta[h],acc);
        }
    }
    grid.sync(); PROFT(5);
    ...
    // slice in registers, so global S is read ONCE and written ONCE.
    {
        ...
        for(int wjob=blockIdx.x; wjob<total; wjob+=gridDim.x){
            ...
            if(tid==0) betas=1.f/(1.f+expf(-beta[h]));
            __syncthreads();
...
__device__ void mla_attn(const int64_t* W, const int64_t* St, int L, float* hid, float* scr, cg::grid_group& grid){
    ...
    // per-warp shared row; qaT read from L2 (32KB, resident). qrope in shared.
    {
        ...
        // 2 tokens/warp, BOTH ckv read from global (no shared staging) so shmem
        // stays minimal (only qr_s + kr) -> keeps high occupancy AND amortizes
        // each qaT load across 2 tokens. lane==head; each lane reads its own head
        // column of qaT and both tokens' ckv[:,kk] via bf162 from L2.
        int stride1=gridDim.x*NWARP;
        for(int l=blockIdx.x*NWARP+warp; l<Lt; l+=stride1*2){
            int l2=l+stride1; bool has2=(l2<Lt);
            const __nv_bfloat162* ckvA=(const __nv_bfloat162*)(nckv+(size_t)l*L0);
            const __nv_bfloat162* ckvB=(const __nv_bfloat162*)(nckv+(size_t)(has2?l2:l)*L0);
            const bf16* krA=nkrope+(size_t)l*QR;
            const bf16* krB=nkrope+(size_t)(has2?l2:l)*QR;
            float a=0,a2=0;
            #pragma unroll 1
            for(int kk=0;kk<L0;kk+=2){ int k2=kk>>1;
                float q0=b2f(__ldg(&qaT_g[(size_t)kk*H+h])), q1=b2f(__ldg(&qaT_g[(size_t)(kk+1)*H+h]));
                float2 c=__bfloat1622float2(__ldg(&ckvA[k2])); a+=q0*c.x+q1*c.y;
                float2 cb=__bfloat1622float2(__ldg(&ckvB[k2])); a2+=q0*cb.x+q1*cb.y; }
            #pragma unroll
            for(int dd=0;dd<QR;dd++){ float qd=b2f(qr_s[h*QR+dd]); a+=qd*b2f(__ldg(&krA[dd])); a2+=qd*b2f(__ldg(&krB[dd])); }
        ...
        if(lseg < bph){
            ...
            for(int t=tid; t<L0; t+=NT){
                ...
                for(int li=0;li<seglen;li++){
                    float ckl=b2f(__ldcs(&nckv[(size_t)(l0+li)*L0+kk])); const float* pr=ps+li*H+h0;
                    #pragma unroll
...
__device__ void moe(const int64_t* W, float* hid, float* scr, cg::grid_group& grid){
    ...
    // top-k lives in SHARED per block (computed redundantly by every block from
    // the published router logits -> removes the grid.sync after top-8).
    __shared__ int topi[16]; __shared__ float topw[NEXP+1];
    ...
    rmsnorm_b0(hid,(const bf16*)W[0],xn);
    // zero router accumulator (block 0) then publish xn AND zeros together.
    if(blockIdx.x==0){ for(int e=tid;e<E;e+=NT) rout[e]=0.f; }
    grid.sync();
    // router logits = xn @ router[E,D]. SPLIT-D across warps: NCH chunks per
    // expert -> E*NCH warps engaged (vs E) to cut serial reduce latency. Each
    // warp does a partial dot over its D-chunk, warp-reduces, atomicAdds to rout.
    {
        const bf16* rw=(const bf16*)W[1];
        const int NCH=8; const int CH=(D+NCH-1)/NCH;   // 288 elems/chunk
        int warp=(blockIdx.x*NT+tid)>>5, lane=tid&31;
        int nwarp=GRID>>5; int njobs=E*NCH;
        for(int job=warp; job<njobs; job+=nwarp){
            int e=job%E, ch=job/E; int i0=ch*CH, i1=min(D,i0+CH);
            const bf16* rwe=rw+e*D; float acc=0.f;
            #pragma unroll 1
            for(int i=i0+lane;i<i1;i+=32) acc+=xn[i]*b2f(__ldg(&rwe[i]));
            #pragma unroll
            for(int o=16;o>0;o>>=1) acc+=__shfl_xor_sync(0xffffffff,acc,o);
            if(lane==0) atomicAdd(&rout[e],acc);
        }
    }
    grid.sync(); PROFT(22);
    // softmax + top-8 by warp 0 of EVERY block (E=64), results in shared -> no
    // grid.sync needed to publish (each block already has its own copy).
    if(tid<32){
    ...
    PROFT(24);
    // ---- batched down with INLINE silu staging: reads gate+up, no silu pass ----
    if(tid<NEXP){
    ...
    __syncthreads();
    bgemv_i4_silu(gbuf, NEXP*MI, wq, sc, zr, dbuf, MI, D, NEXP, grid); PROFT(25);
    // weighted sum: out[i]=sum_j topw[j]*dbuf[j*D+i]. topw already in shared.
    float* tws=topw;
    int D4=D>>2;
...
__global__ void __launch_bounds__(NT,LB) mega_kernel(const int64_t* W, const int* Woff, const int64_t* St,
                            ...
                            const bf16* __restrict__ hidin, bf16* __restrict__ hidout){
    cg::grid_group grid=cg::this_grid();
    g_part = scr_base;                 // partial-sum buffer
    float* scr = scr_base + PART_FLOATS;
    int Soff[4]; Soff[0]=s0; Soff[1]=s1; Soff[2]=s2; Soff[3]=s3;
    // FUSED input convert: bf16 token -> float working buffer (block 0 only;
    // the first layer's rmsnorm_b0 reads hid on block 0, and its grid.sync
    // publishes hid to the rest of the grid -> no extra launch, no extra sync).
    if(blockIdx.x==0){ for(int i=threadIdx.x;i<D;i+=NT) hid[i]=b2f(hidin[i]); }
    __syncthreads();
    ...
    // FUSED output convert: float working buffer -> bf16 output (all blocks;
    // kernel exit is an implicit device sync so no trailing grid.sync needed).
    for(int i=blockIdx.x*NT+threadIdx.x;i<D;i+=GRID) hidout[i]=__float2bfloat16(hid[i]);
}
...
void mega_launch(int64_t Wtab, int64_t Woff, int64_t Stab,
    ...
    const bf16* hidin=(const bf16*)hidin_ptr; bf16* hidout=(bf16*)hidout_ptr;
    ...
    void* args[]={(void*)&W,(void*)&Wo,(void*)&St,(void*)&hid,(void*)&scr,
                  ...
                  (void*)&hidin,(void*)&hidout};
    size_t shmem = 4128*sizeof(float);   // max of: cvec ps[<=4128 f], scores stage
    { size_t sc_sh=(size_t)(H*QR + 16)*sizeof(bf16); if(sc_sh>shmem) shmem=sc_sh; }
