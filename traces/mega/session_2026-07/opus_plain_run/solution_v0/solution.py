...
class Model(nn.Module):
    ...
    def _ensure_scratch(self, Lt):
        ...
        ) + 4096 + (1 << 19)  # + PART_FLOATS for deterministic split-K partials
    ...
    def step(self, hidden, state):
        ...
        if self._hid is None:
            ...
            self._out = torch.empty(cfg.hidden, dtype=torch.bfloat16, device="cuda")
        hid = self._hid
        hid.copy_(hidden)
        ...
        # meta: persistent CPU-pinned buffer copied once to a persistent GPU buffer.
        if self._meta_gpu is None:
            self._meta_cpu = torch.zeros(8, dtype=torch.int32, pin_memory=True)
            self._meta_gpu = torch.zeros(8, dtype=torch.int32, device="cuda")
        if rebuild:
            for j, v in enumerate(self._Soff):
                self._meta_cpu[j] = v
        self._meta_cpu[4] = L
        self._meta_cpu[5] = nblk
        self._meta_cpu[6] = do_last_moe
        self._meta_gpu.copy_(self._meta_cpu, non_blocking=True)
        self._ensure_scratch(L + 1)
        self._mod.mega_launch(
            self._Wt.data_ptr(), self._Woff.data_ptr(), self._St.data_ptr(),
            self._meta_gpu.data_ptr(), hid.data_ptr(), self._scr.data_ptr(),
        )
