...
## Frontier status — goal-directed Generation 11

**The canonical eight-selector residue template fails adverse graded injectivity at depth one; the ramified FRONTIER remains open.** Only repaired Pro proposal 1 survived. Its causal mechanism was skew-Rees exactness over `F_289`; the expected move was removal of every false-boundary grade-zero class, and any zero-defect subthreshold affine class was the falsifier.

`experiments/verify_f289_nand_affine_grade_zero_attack.py` emits the complete canonical NAND matrix and exhausts all four false boundaries. Each has a unique integral signed representation using only legal columns. The cheapest three have zero residual and anchor energy 16; their base-field coefficients are Frobenius-fixed, so they remain grade zero. Since the minimum trace energy of a nonzero element of the prime above 17 is 34, the canonical module fails before maximal-order lifting or depth-two composition. This finite result kills the template, not other quaternion gate libraries or the ramified adverse-filtration lemma.

### Goal-directed Generation 12

**A redundant `N=8` NAND module survives the complete local gate; the ramified FRONTIER remains unproved.** Both reviews retained only the redundant-signature mutation. Its causal mechanism was raising unavoidable affine false representatives by repeating selected binary signatures; the expected move was exact false energy at least the independently derived prime threshold and ratio above `17/16`, while any cheaper unrestricted fiber was the falsifier.

`experiments/verify_redundant_signature_nand_survivor.py` exhausts all 490,314 length-eight multisets and finds 13,457 saturated local passes. The best complete emitted matrix is unimodular, with legal energy 8 and exact false energies `160,64,56,56`. The same verifier constructs an explicit maximal order in `(-3,-17)`, its two-sided prime above 17, and certifies minimum nonzero trace energy 34. Thus the candidate passes `56>=34` and `56/8>17/16`. This is finite depth-one evidence only; COPY, depth-two coupled optimization, and an all-depth filtration proof remain mandatory.
