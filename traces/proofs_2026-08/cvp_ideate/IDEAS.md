...
**Unresolved parameter wall.** Naive phase replication changes length to `1+ell(L-1)` and YES weight to `1+ell(d-1)` before folding, so the resulting length remains `Omega(ell L^2)`—worse than the unreplicated tensor. To amplify `(q+2)/(q+1)`, the exponent after `k` squarings is `2^k`; useful iteration requires an intrinsic large free symmetry at every level and an invariant sparse YES word without paying the group order in padding. A symmetry-replenishing balanced/lifted product remains unconstructed.
...
**CURRENT STATUS.** The clean invariant ladder conserves a vanishing base exponent; the tested non-invariant ladder fails to compress completeness. A surviving mutation needs structured autocorrelation cancellation in a code whose true pointed distance remains large, not direct-sum sheets.
