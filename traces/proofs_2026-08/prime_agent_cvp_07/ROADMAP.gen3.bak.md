...
## Frontier status — Generation 1 synthesis

The cross-review-surviving mutated finite adverse-transducer proposal (Pro 2 / Fable 1) was tested. Its mechanism needs every adverse seam to have a nonzero leading \(P\)-class, a lift-independent finite transition, and a potential certificate with sufficient average gain. The expected move was to reduce Q2 to a checked finite table plus a universal telescope theorem. Falsification was any grade-zero adverse seed, carry-dependent successor, or zero-gain signed splice.

The explicit affine realization fails the first gate. The natural determinant-one NAND defect map has exact zero-residual false fibers `010`, `100`, and `111` at squared energies 64, 56, and 56. Moreover, every one of the 378 saturated binary-signature COPY multisets of ranks 2–8 admits the zero-residual `false111-COPY11-false111` splice in both orientations, at total squared energy `112+N` (114–120). These are finite counterexamples only; they do not exclude a new emitted quaternionic transfer coordinate.

`lean/Verify_transducer_potential.lean` proves the reusable universal implication that a local integer potential certificate telescopes along every finite walk, with a bounded-potential corollary, and that one valuation gain per four binary levels beats the binary scale because `17>2^4`. It does not construct a COPY tile, transition graph, lumpability proof, or certificate for this NAND.

Thus Q2 and the COPY part of Q1 remain open. The next admissible experiment must freeze an augmented integral quaternionic NAND/COPY coupling whose new transfer coordinate is nonzero on the displayed `111` selector, then exhaust all normalized \(P^2\) states and their \(P^3\) lifts, including DROP and internal zero-boundary kernels. Any carry split or repeated zero-gain state kills that candidate; a finite pass still does not establish Q2.
