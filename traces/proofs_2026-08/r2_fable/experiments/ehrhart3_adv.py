# Question: adversarial test of the recursive n=3 certificate. Over feasible bodies
# (barycenter 0, int cap Z^3={0}), MAXIMIZE  min_ell max( (vol/area_ell)/(4/3)^3 , area_ell/4.5 ).
# If sup <= 1 the two-part certificate covers all of n=3 (numerically). Value >1+eps = counterexample.
...
def objective(flat):
    ...
    cv, _ = cert_value(pts0, vol)
    if not np.isfinite(cv): return 1e6
    # maximize cert value, keep feasible, prefer bigger bodies (cv includes vol)
    return -cv + 300.0*pen
...
for trial in range(10**9):
    ...
    cv, ell = cert_value(pts0, vol)
    if cv > best_global[0]:
        best_global = (cv, pts0.copy())
        print(f"trial {trial}: certval={cv:.5f} vol={vol:.4f} ell={ell}", flush=True)
        if cv > 1.0 + 1e-6:
            np.set_printoptions(precision=5, suppress=True)
            print("POTENTIAL COUNTEREXAMPLE vertices:"); print(pts0[hull(pts0).vertices], flush=True)
