#!/usr/bin/env python3
# Question: can the verified 3-block/13-color rule be extended by one seed block using fewer than 18 colors?
import argparse,json,itertools,random,time
ap=argparse.ArgumentParser();ap.add_argument('-q',type=int,default=17);ap.add_argument('--steps',type=int,default=500000);ap.add_argument('--restarts',type=int,default=10);ap.add_argument('--seed',type=int,default=1);ap.add_argument('--out',default='experiments/extended_rule.json');args=ap.parse_args();rng=random.Random(args.seed)
...
for rels in itertools.product(R5,repeat=3):
 ss=[tuple(rels[j][h] for j in range(3)) for h in range(3)];R13.add(tuple(0 if not any(s) else mp[s] for s in ss))
states=[s for s in itertools.product(range(14),range(6)) if any(s)];idx={s:i for i,s in enumerate(states)};T=set()
...
print('relations',len(R13),len(R5),'states',len(states),'triples',len(T),flush=True);best=10**9;t0=time.time()
